...
SQL_H5P_DEPENDENCIES = [
    {"machineName": "H5P.SQLQuestion", "majorVersion": 6, "minorVersion": 90},
...
class SQLQuestion(H5PContentType):
    """H5P.SQLQuestion — SQL IDE activity."""

    MACHINE_NAME: ClassVar[str] = "H5P.SQLQuestion"
    ...
    @classmethod
    def from_block(cls, block: object) -> SQLQuestion:
        h5p_content = getattr(block, "h5p_content", None)
        database = ""
        if isinstance(h5p_content, dict):
            db_settings = h5p_content.get("databaseSettings", {})
            if isinstance(db_settings, dict):
                database = str(db_settings.get("selectDatabase") or db_settings.get("select_db") or "")
        return cls(
            ...
            database=getattr(block, "database", database),
    ...
    def render_mdx_tag(
        ...
        *,
        ...
        if self.database:
            lines.append(render_tag_attribute("database", self.database))
        ...
        for role, body in [
            ...
            if not body:
                continue
            lines += [
                f"```sql question:{self.identifier} {role}",
                body,
                "```",
                "",
            ]
    ...
    def build_metadata_json(self, library_json: dict[str, object]) -> dict[str, object]:
        return {
            ...
            "embedTypes": ["iframe"],
            "license": "U",
            "preloadedDependencies": SQL_H5P_DEPENDENCIES,
        }
